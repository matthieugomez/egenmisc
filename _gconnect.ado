program define _gconnect

	gettoken type 0 : 0
	gettoken g    0 : 0
	gettoken eqs  0 : 0

	program define connect
		syntax varlist [, [Missing id(varname)]
		...
		quietly {
			...
			count if missing(`by')
			if `r(N)'>0{
				display "id variable should not be missing" as error
			}

			/* transform each variable in id */
			sort `by'
			tempvar temp
			by `by': gen `temp' = _n == 1
			...
			foreach v of varlist `varlist' {
				tempvar g`v'
				bys `by' `v': gen long g`v' = _n == 1
				replace g`v' = sum(g`v')
				if "`missing'"==""{
					replace g`v' = . if missing(`v')
				}
			}
			tempvar temp ttemp tttemp
			tokenize `varlist'
			while "`1'"~=""{
				a2group2, individual(`temp') unit(`g`1'') groupvar(`ttemp')
				replace `ttemp' = - `ttemp'
				bys temp (ttemp): gen `tttemp' = `tttemp'[1]
				replace `temp' = - `tttemp'
				drop `ttemp' `tttemp'
				macro shift
			}
		...
		tempvar temp
		bys `gen': gen `temp' = _n == 1
		replace `temp' = sum(`temp')
		local genn `=`temp'[_N]'
		dis as text "Initial groups = "in ye %9.0gc `byn'
